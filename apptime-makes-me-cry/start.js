...
const reporter = ({ state, stats, options }) => {
  ...
  console.log(`Dev server listening at ${chalk.cyan.bold(`http://${process.env.DEV_LISTEN_ADDR || 'localhost'}:${port}`)}`);
  console.log();
...
app.listen(port, process.env.DEV_LISTEN_ADDR || 'localhost', err => {
  if (err) {
    console.error(err);
    return;
  }
